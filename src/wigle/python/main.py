...
# from https://code.activestate.com/recipes/578637-wigle-wifi-geolocation/
    ...
    def send_query(self, mac_address):
        response = self.agent.post(url='https://wigle.net/api/v1/jsonLocation',
                       data={'netid': mac_address,
                             'Query2': 'Query'})
        if hasattr(response.json, '__call__'): return response.json()
        print(response.json)
...
if __name__ == "__main__":
    ...
    wa = WigleAgent(username, password)
    print wa.get_lat_lng('00:1C:0E:42:79:43')
    print wa.get_lat_lng('00:22:55:DF:C8:01')
